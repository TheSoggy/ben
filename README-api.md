...
same `DDSSolver.solve` call path the play/lead/claim endpoints use, and
returns 200 only when the solver produces the known-correct answer. A
reachability probe on `/` cannot see a solver-only outage — `/` stayed 200
through the 2026-08-05 incident while every double-dummy endpoint was
failing.

Scope: this exercises `DDSSolver.solve` → `SolveAllBoards`. `/double_dummy`
(`CalcDDtablePBN`) and `/solve_board` (`SolveBoardPBN`) call the DDS library
directly and need their own probes (the parent repo's smoke script covers
them). Exempt from rate limiting. Cost is well under a millisecond per call.
...
```json
{"status": "ok", "elapsed_ms": 0.4}
```

**Response (unhealthy, 503):**
```json
{"status": "unhealthy", "error": "solve raised TypeError: ..."}
```
