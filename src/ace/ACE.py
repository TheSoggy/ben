...
This module provides an interface to the Ace library, which implements
...
class ACEDLL:
    ...
    def __init__(self, models, northhand, southhand, contract, is_decl_vuln, sampler, verbose):
        ...
        # ACE configuration
        self.search_duration = getattr(models, 'ace_search_duration', 5000)  # ms
        self.iterations = getattr(models, 'ace_iterations', 0)  # 0 = unlimited
    ...
    def set_hcp_constraints(self, min_lho, max_lho, min_rho, max_rho, quality):
        ...
        if self.passed_lho and self.passed_hand_hcp_cap > 0:
            ...
            self.lho_constraints[9] = min(self.lho_constraints[9], cap)
    ...
    def _run_search_and_evaluate(self, game, depth):
        ...
        # Build Config with Exploration and Limiter
        ...
        # Evaluate returns List<Evaluation> sorted best-first
    ...
    def nextplay(self, player_i, shown_out_suits, missing_cards):
        """Find the best card to play using ACE engine"""
        t_start = time.time()
        ...
        try:
            ...
            logger.info("ACE Request: PBN=%s Contract=%s Played=%s Constraints=[RHO:%s LHO:%s] Depth=%s",
            ...
            try:
                ...
                logger.info("ACE Response: %s Time=%.1fs", card_result, elapsed)
            ...
            logger.error("ACE nextplay error: %s", e, exc_info=True)
